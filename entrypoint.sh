...
echo "Starting Easyedit with PUID=$PUID PGID=$PGID"

addgroup -g "$PGID" easyedit 2>/dev/null || true
adduser -u "$PUID" -G easyedit -D -H -s /sbin/nologin easyedit 2>/dev/null || true
...
if [ "${ROBOTS_NOINDEX:-false}" = "true" ] && [ -x /usr/local/bin/easyedit-noindex.sh ]; then
    NOINDEX_HTML_ROOT=/usr/share/nginx/html /usr/local/bin/easyedit-noindex.sh || true
fi
