...
Easyedit can be self-hosted on your own infrastructure. This guide covers various deployment options.

## Quick Start with Docker / Podman

The fastest way to self-host Easyedit:
...
>
> Use `http://localhost` only for same-device testing. If users access Easyedit through a LAN IP or hostname, terminate it with HTTPS.

```bash
# Docker
docker run -d -p 3000:8080 ghcr.io/easyedit/easyedit:latest

# Podman
podman run -d -p 3000:8080 ghcr.io/easyedit/easyedit:latest
```
...
Run Easyedit as a systemd service. Create `~/.config/containers/systemd/easyedit.container`:
...
systemctl --user enable --now easyedit
```
...
```bash
# Clone and build
git clone https://github.com/easyedit/easyedit.git
cd easyedit
npm install
npm run build
...
See [SIMPLE_MODE.md](https://github.com/easyedit/easyedit/blob/main/SIMPLE_MODE.md) for full details.
...
Replace the default Easyedit logo, name, and footer text with your own at build time:

| Variable           | Description                           | Default                                 |
| ------------------ | ------------------------------------- | --------------------------------------- |
| `VITE_BRAND_NAME`  | Brand name shown in header and footer | `Easyedit`                              |
...
Easyedit **does not bundle** AGPL-licensed processing libraries in its source code, but **pre-configures CDN URLs** so all features work out of the box — no manual setup needed.
...
```bash
VITE_WASM_PYMUPDF_URL=https://cdn.jsdelivr.net/npm/@easyedit/pymupdf-wasm@0.11.16/
VITE_WASM_GS_URL=https://cdn.jsdelivr.net/npm/@easyedit/gs-wasm@0.1.1/assets/
...
| `--image-name <name>`          | Docker image tag                                 | `easyedit`                        |
| `--output-dir <path>`          | Output bundle directory                          | `./easyedit-airgap-bundle`        |
...
WASM files must be served from the **same origin** as the Easyedit app. Web Workers use `importScripts()` which cannot load scripts cross-origin. For example, if Easyedit runs at `https://internal.example.com`, the WASM base URL should also be `https://internal.example.com/wasm`.
:::
...
tar xzf easyedit-pymupdf-wasm-0.11.14.tgz -C ./wasm/pymupdf --strip-components=1
tar xzf easyedit-gs-wasm-*.tgz -C ./wasm/gs --strip-components=1
...
If you need to serve AGPL WASM files with proper CORS headers, you can deploy a simple proxy. See the [Cloudflare WASM Proxy guide](https://github.com/easyedit/easyedit/blob/main/cloudflare/WASM-PROXY.md) for an example implementation.

::: tip Why Separate?
This separation ensures:

- Clear legal compliance for commercial users
- Easyedit's core remains under its dual-license (AGPL-3.0 / Commercial)
...
::: tip
Easyedit is a static site—there's no database or backend server required!
:::
