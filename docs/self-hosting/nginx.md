...
Host Easyedit on your own server using Nginx.
...
Create `/etc/nginx/sites-available/easyedit`:
...
```bash
sudo ln -s /etc/nginx/sites-available/easyedit /etc/nginx/sites-enabled/
sudo nginx -t
