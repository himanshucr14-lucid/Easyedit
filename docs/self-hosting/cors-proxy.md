...
Additionally, many CA servers serve certificates over plain HTTP. When your Easyedit instance is hosted over HTTPS, browsers block these HTTP requests (mixed content policy). The CORS proxy resolves both issues by routing requests through an HTTPS endpoint with proper headers.
...
If you're self-hosting Easyedit, you'll need to deploy your own CORS proxy for digital signatures to work with certificates that require chain fetching.
...
3. **Clone Easyedit and update allowed origins**:

   ```bash
   git clone https://github.com/easyedit/easyedit.git
   cd easyedit/cloudflare
   ```
   ...
   ::: warning Important
   Without this change, the proxy will reject all requests from your site with a **403 Forbidden** error. The default only allows requests from `easyedit.com`.
   :::
...
4. **Deploy the proxy**:
   ...
   Note your worker URL (e.g., `https://easyedit-cors-proxy.your-subdomain.workers.dev`).

5. **Rebuild Easyedit with the proxy URL**:
