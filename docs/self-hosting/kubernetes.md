...
>
> The official Easyedit nginx images include these headers. In Kubernetes, **Ingress/Gateway controllers are also reverse proxies**, so ensure these headers are preserved (or add them at the edge).

## Prereqs

- Kubernetes cluster
- Helm v3
- A Easyedit nginx image (e.g. `ghcr.io/easyedit/easyedit:<tag>`) that serves on **port 8080**
...
```bash
kubectl create namespace easyedit

helm upgrade --install easyedit /path/to/easyedit/chart \
  --namespace easyedit \
  --set image.repository=ghcr.io/easyedit/easyedit \
...
helm upgrade --install easyedit oci://ghcr.io/$GHCR_USERNAME/charts/easyedit \
  --namespace easyedit \
  --create-namespace \
  --version 0.1.0 \
  --set image.repository=ghcr.io/easyedit/easyedit \
...
```bash
kubectl -n easyedit port-forward deploy/easyedit 8080:8080
```
...
Easyedit’s nginx config sets the required response headers. Most Ingress/Gateway controllers **pass upstream response headers through unchanged**.
