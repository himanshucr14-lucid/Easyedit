...
The easiest way to self-host Easyedit in a production environment.
...
services:
  easyedit:
    image: ghcr.io/easyedit/easyedit:latest
...
```bash
docker build -t easyedit:custom .
docker run -d -p 3000:8080 easyedit:custom
```
...
| `VITE_WASM_PYMUPDF_URL`              | PyMuPDF WASM module URL                     | `https://cdn.jsdelivr.net/npm/@easyedit/pymupdf-wasm@0.11.16/` |
| `VITE_WASM_GS_URL`                   | Ghostscript WASM module URL                 | `https://cdn.jsdelivr.net/npm/@easyedit/gs-wasm@0.1.1/assets/` |
...
| `VITE_BRAND_NAME`                    | Custom brand name                           | `Easyedit`                                                     |
| `VITE_BRAND_LOGO`                    | Logo path relative to `public/`             | `images/favicon-no-bg.svg`                                     |
| `VITE_FOOTER_TEXT`                   | Custom footer/copyright text                | `© 2026 Easyedit. All rights reserved.`                        |
...
- Air-gapped deployments that override all three `VITE_WASM_*_URL` values also get the public `cdn.jsdelivr.net` removed from CSP (each default is replaced, not appended). Similarly, setting `VITE_CORS_PROXY_URL` replaces the public `easyedit-cors-proxy.easyedit.workers.dev` default.
...
docker build --build-arg VITE_DEFAULT_LANGUAGE=fr -t easyedit .
docker run -d -p 3000:8080 easyedit
```

### Custom Branding

Replace the default Easyedit logo, name, and footer text with your own. Place your logo file in the `public/` folder (or use an existing image), then pass the branding variables at build time:
...
Tool IDs are the page URL without `.html`. For example, if the tool lives at `easyedit.com/edit-pdf.html`, the ID is `edit-pdf`.

#### Finding Tool IDs

The easiest way: open any tool in Easyedit and look at the URL. The last part of the path (without `.html`) is the tool ID.
...
npm pack @easyedit/pymupdf-wasm@0.11.14
npm pack @easyedit/gs-wasm
...
# 4. Export the image
docker save easyedit -o easyedit.tar

# 5. Transfer easyedit.tar + the .tgz packages + tesseract-langdata/ + ocr-fonts/ into the air-gapped network

# 6. Inside the air-gapped network — load and run
docker load -i easyedit.tar
...
tar xzf easyedit-pymupdf-wasm-0.11.14.tgz -C /var/www/wasm/pymupdf --strip-components=1
tar xzf easyedit-gs-wasm-*.tgz -C /var/www/wasm/gs --strip-components=1
...
# Run Easyedit
docker run -d -p 3000:8080 --restart unless-stopped easyedit
```
...
By default, Easyedit listens on port `8080` inside the container. To change this, set the `PORT` environment variable at runtime:

```bash
docker run -p 3000:9090 -e PORT=9090 ghcr.io/easyedit/easyedit:latest
```
...
For environments that require running as a specific non-root user (NAS devices, Kubernetes with security contexts, organizational policies), Easyedit provides a separate Dockerfile with LSIO-style PUID/PGID support.

### Build and Run

```bash
# Build the non-root image
docker build -f Dockerfile.nonroot -t easyedit-nonroot .
...
services:
  ...
  easyedit:
    ...
    labels:
      - 'traefik.enable=true'
      - 'traefik.http.routers.easyedit.rule=Host(`pdf.example.com`)'
      - 'traefik.http.routers.easyedit.entrypoints=websecure'
      - 'traefik.http.routers.easyedit.tls.certresolver=letsencrypt'
      - 'traefik.http.services.easyedit.loadbalancer.server.port=8080'
      # Required headers for SharedArrayBuffer (LibreOffice WASM)
      - 'traefik.http.routers.easyedit.middlewares=easyedit-headers'
      - 'traefik.http.middlewares.easyedit-headers.headers.customresponseheaders.Cross-Origin-Opener-Policy=same-origin'
      - 'traefik.http.middlewares.easyedit-headers.headers.customresponseheaders.Cross-Origin-Embedder-Policy=require-corp'
    restart: unless-stopped
```
...
Create a container unit file at `~/.config/containers/systemd/easyedit.container` (user) or `/etc/containers/systemd/easyedit.container` (system):

```ini
[Unit]
Description=Easyedit - Privacy-first PDF toolkit
After=network-online.target
Wants=network-online.target

[Container]
Image=ghcr.io/easyedit/easyedit:latest
ContainerName=easyedit
...
# Start the service
systemctl --user start easyedit

# Enable on boot
systemctl --user enable easyedit

# Check status
systemctl --user status easyedit

# View logs
journalctl --user -u easyedit -f
```
...
For Simple Mode deployment, create `easyedit-simple.container`:

```ini
[Unit]
Description=Easyedit Simple Mode - Clean PDF toolkit
After=network-online.target
Wants=network-online.target
...
```ini
[Unit]
Description=Easyedit with health monitoring
After=network-online.target
Wants=network-online.target
...
For custom network configuration, create a network file `easyedit.network`:
