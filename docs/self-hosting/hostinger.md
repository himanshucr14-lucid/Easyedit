...
[Hostinger](https://hostinger.com) is a popular shared hosting provider. This guide covers deploying Easyedit to Hostinger's shared hosting.
...
RewriteCond %{HTTP_HOST} ^easyedit\.com [NC]
RewriteRule ^(.*)$ https://www.easyedit.com/$1 [L,R=301]
...
Make sure the `.html` extension rule and language routes are correctly configured. Easyedit uses static HTML files, not SPA routing:
