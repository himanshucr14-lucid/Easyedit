...
Deploys **Easyedit** as a **single NGINX container** serving the static frontend.
...
```bash
helm install easyedit ./chart
kubectl port-forward deploy/easyedit 8080:8080
...
- **`image.repository`**: container image repo (default: `ghcr.io/easyedit/easyedit-simple`)
- **`image.tag`**: image tag (default: `Chart.appVersion`)
- **`image.pullPolicy`**: default `IfNotPresent`

### Ports

- **`containerPort`**: container listen port (**8080** for the Easyedit nginx image)
...
# produces easyedit-<version>.tgz
helm push easyedit-*.tgz oci://ghcr.io/$GHCR_USERNAME/charts
```
...
```bash
helm upgrade --install easyedit oci://ghcr.io/$GHCR_USERNAME/charts/easyedit --version 1.0.0
```
