...
# ============================================================
# Easyedit Air-Gapped Deployment Preparation Script
# ============================================================
...
# contains everything needed to deploy Easyedit offline.
#
# Usage:
...
WASM_BASE_URL=""
IMAGE_NAME="easyedit"
OUTPUT_DIR="./easyedit-airgap-bundle"
SIMPLE_MODE=""
BASE_URL=""
COMPRESSION_MODE=""
LANGUAGE=""
...
usage() {
  cat <<'EOF'
Easyedit Air-Gapped Deployment Preparation
...
OPTIONS:
  --image-name <name>     Docker image name          (default: easyedit)
  --output-dir <path>     Output bundle directory     (default: ./easyedit-airgap-bundle)
...
if [ ! -f "package.json" ] || [ ! -f "src/js/const/cdn-version.ts" ]; then
  error "This script must be run from the Easyedit project root."
...
interactive_mode() {
  echo ""
  echo -e "${BOLD}============================================================${NC}"
  echo -e "${BOLD}  Easyedit Air-Gapped Deployment Preparation${NC}"
  ...
  echo "      https://cdn.mycompany.local/easyedit"
  echo ""
  while true; do
  ...
  echo "    Replace the default Easyedit name, logo, and footer text."
  read -r -p "    Brand name [Easyedit]: " input
  BRAND_NAME="${input:-}"
  if [ -n "$BRAND_NAME" ]; then
    ...
    BRAND_LOGO="${input:-}"
    read -r -p "    Footer text [© 2026 Easyedit. All rights reserved.]: " input
    FOOTER_TEXT="${input:-}"
  fi
  echo ""
  ...
  echo "  Brand Name:     ${BRAND_NAME:-Easyedit (default)}"
...
echo ""
echo -e "${BOLD}============================================================${NC}"
echo -e "${BOLD}  Easyedit Air-Gapped Bundle Preparation${NC}"
...
if ls "$OUTPUT_DIR"/*.tgz "$OUTPUT_DIR"/easyedit.tar "$OUTPUT_DIR"/setup.sh 2>/dev/null | head -1 &>/dev/null; then
...
if [ "$SKIP_WASM" = true ]; then
  ...
  wasm_missing=false
  if ! ls "$OUTPUT_DIR"/easyedit-pymupdf-wasm-*.tgz &>/dev/null; then
    error "Missing: easyedit-pymupdf-wasm-*.tgz"
    wasm_missing=true
  fi
  if ! ls "$OUTPUT_DIR"/easyedit-gs-wasm-*.tgz &>/dev/null; then
    error "Missing: easyedit-gs-wasm-*.tgz"
    wasm_missing=true
  fi
  ...
  info "Downloading @easyedit/pymupdf-wasm@${PYMUPDF_VERSION}..."
  if ! (cd "$WASM_TMP" && npm pack "@easyedit/pymupdf-wasm@${PYMUPDF_VERSION}" --quiet 2>&1); then
    error "Failed to download @easyedit/pymupdf-wasm@${PYMUPDF_VERSION}"
    error "Check your internet connection and that the package exists on npm."
    exit 1
  fi

  info "Downloading @easyedit/gs-wasm@${GS_VERSION}..."
  if ! (cd "$WASM_TMP" && npm pack "@easyedit/gs-wasm@${GS_VERSION}" --quiet 2>&1); then
    error "Failed to download @easyedit/gs-wasm@${GS_VERSION}"
    error "Check your internet connection and that the package exists on npm."
    exit 1
  fi
  ...
  info "  PyMuPDF:      $(filesize "$OUTPUT_DIR"/easyedit-pymupdf-wasm-*.tgz)"
  info "  Ghostscript:  $(filesize "$OUTPUT_DIR"/easyedit-gs-wasm-*.tgz)"
...
if [ "$SKIP_DOCKER" = true ]; then
  ...
  if [ -f "$OUTPUT_DIR/easyedit.tar" ]; then
    success "Reusing existing easyedit.tar"
  elif docker image inspect "$IMAGE_NAME" &>/dev/null; then
    step "Exporting existing Docker image"
    docker save "$IMAGE_NAME" -o "$OUTPUT_DIR/easyedit.tar"
    success "Exported: $(filesize "$OUTPUT_DIR/easyedit.tar")"
  else
    warn "No Docker image '${IMAGE_NAME}' found and no easyedit.tar in output."
...
# ============================================================
# Easyedit Air-Gapped Setup Script
# Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
# Easyedit v${APP_VERSION}
# ============================================================
...
echo ""
echo "============================================================"
echo "  Easyedit Air-Gapped Setup"
...
if [ -f "\${SCRIPT_DIR}/easyedit.tar" ]; then
  docker load -i "\${SCRIPT_DIR}/easyedit.tar"
  echo "  Docker image '\${IMAGE_NAME}' loaded."
else
  echo "  WARNING: easyedit.tar not found. Skipping Docker load."
...
tar xzf "\${SCRIPT_DIR}"/easyedit-pymupdf-wasm-*.tgz -C "\${WASM_DIR}/pymupdf" --strip-components=1

# Ghostscript: browser expects gs.js and gs.wasm at root
echo "  Extracting Ghostscript..."
TEMP_GS="\$(mktemp -d)"
tar xzf "\${SCRIPT_DIR}"/easyedit-gs-wasm-*.tgz -C "\${TEMP_GS}"
...
# --- Step 3: Start Easyedit ---
echo ""
echo "[3/3] Ready to start Easyedit"
echo ""
echo "  To start manually:"
echo "    docker run -d --name easyedit -p \${DOCKER_PORT}:8080 --restart unless-stopped \${IMAGE_NAME}"
echo ""
echo "  Then open: http://localhost:\${DOCKER_PORT}"
echo ""

read -r -p "Start Easyedit now? (y/N): " REPLY
if [[ "\${REPLY:-}" =~ ^[Yy]$ ]]; then
  docker run -d --name easyedit -p "\${DOCKER_PORT}:8080" --restart unless-stopped "\${IMAGE_NAME}"
  echo ""
  echo "  Easyedit is running at http://localhost:\${DOCKER_PORT}"
fi
SETUP_EOF
...
# Easyedit Air-Gapped Deployment Bundle

**Easyedit v${APP_VERSION}** | Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")

## Contents

| File | Description |
| --- | --- |
| \`easyedit.tar\` | Docker image |
| \`easyedit-pymupdf-wasm-${PYMUPDF_VERSION}.tgz\` | PyMuPDF WASM module |
| \`easyedit-gs-wasm-${GS_VERSION}.tgz\` | Ghostscript WASM module |
...
3. Optionally start the Easyedit container
...
# PyMuPDF
tar xzf easyedit-pymupdf-wasm-${PYMUPDF_VERSION}.tgz -C ./wasm/pymupdf --strip-components=1

# Ghostscript (extract assets/ to root)
TEMP_GS=\$(mktemp -d)
tar xzf easyedit-gs-wasm-${GS_VERSION}.tgz -C \$TEMP_GS
...
\`\`\`bash
docker run -d --name easyedit -p 3000:8080 --restart unless-stopped ${IMAGE_NAME}
\`\`\`
